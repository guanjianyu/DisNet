...
from DisNetRNN_3 import DisNet_RNN
import argparse
...
class ObjectDetect:
    ...
    def callback(self, data):
        ...
        with graph.as_default():
            image_zoom, figure = model.detect_image(image)
            ...
            k = cv2.waitKey(1)
            if k == ord("s"):
                cv2.imwrite("image_{}.png".format(i), result)
                cv2.imwrite("figure_{}.png".format(i), figure)
                print("save")
                i += 1
